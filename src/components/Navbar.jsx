...
const NAV_ITEMS = [
  ...
  { label: 'Project', sectionId: 'project-section' },
  { label: 'Tech Stack', sectionId: 'tech-stack-section' },
  { label: 'Contact', sectionId: 'contact-section' },
];

const DARK_SECTION_IDS = ['hero-section', 'about-section', 'infra-section', 'project-section', 'tech-stack-section', 'github-stats-section', 'contact-section'];
    ...
    <nav className={`fixed top-0 left-0 w-full p-4 md:p-6 flex justify-between items-center z-50 pointer-events-none transition-all duration-500`}>
      ...
      <Magnetic>
        <div
          ...
          className={`group pointer-events-auto flex items-center gap-3 px-5 py-2.5 rounded-full backdrop-blur-md border transition-all duration-500 cursor-pointer ${scrolled ? 'bg-black/40 border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)]' : 'bg-transparent border-transparent'}`}
        >
      ...
      <div className="lg:hidden pointer-events-auto relative z-50">
        <button
          ...
          className={`w-11 h-11 flex items-center justify-center rounded-full backdrop-blur-md border transition-all duration-300 ${isMenuOpen ? 'bg-blue-600 border-blue-500 text-white shadow-[0_8px_24px_rgba(37,99,235,0.3)]' : 'bg-white/10 border-white/20 text-white'}`}
        >
      ...
      <GsapPresence>
        {isMenuOpen && (
          ...
          >
            <div className={`absolute inset-0 opacity-[0.1] pointer-events-none [background-size:28px_28px] ${isOnDarkSection ? '[background-image:linear-gradient(to_right,rgba(255,255,255,0.24)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.24)_1px,transparent_1px)]' : '[background-image:linear-gradient(to_right,rgba(255,255,255,0.24)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.24)_1px,transparent_1px)]'}`} />
            <div className="absolute -top-24 -right-16 w-72 h-72 rounded-full bg-blue-600/15 blur-3xl pointer-events-none" />
            <div className={`absolute -bottom-24 -left-10 w-64 h-64 rounded-full blur-3xl pointer-events-none ${isOnDarkSection ? 'bg-white/12' : 'bg-white/12'}`} />
            <div className={`absolute inset-0 pointer-events-none ${isOnDarkSection ? 'bg-[radial-gradient(circle_at_15%_5%,rgba(255,255,255,0.14),rgba(0,0,0,0)_48%)]' : 'bg-[radial-gradient(circle_at_15%_5%,rgba(255,255,255,0.14),rgba(0,0,0,0)_48%)]'}`} />
            ...
            >
              <div className={`w-full max-w-md mx-auto pb-4 border-b flex items-center justify-between ${isOnDarkSection ? 'border-white/20' : 'border-black/15'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-mono text-[10px] tracking-[0.14em] ${isOnDarkSection ? 'text-white/55' : 'text-blue-600/50'}`}>CLOUD.NODE</span>
                </div>
              </div>
              ...
              <Gsap.button
                ...
                className={`w-full max-w-md mx-auto mt-6 h-12 rounded-full text-xs font-bold uppercase tracking-[0.16em] flex items-center justify-center gap-2 shadow-[0_10px_24px_rgba(37,99,235,0.2)] ${isOnDarkSection ? 'bg-white text-blue-600' : 'bg-blue-600 text-white'}`}
              >
                Connect Now
      ...
      <div className={`hidden lg:flex items-center pointer-events-auto absolute left-1/2 -translate-x-1/2 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${scrolled ? 'top-6 scale-100' : 'top-8 scale-105'}`}>
        <div className={`flex items-center p-1.5 backdrop-blur-xl border shadow-[0_12px_40px_rgba(0,0,0,0.3)] rounded-full relative transition-colors duration-500 ${isOnDarkSection ? 'bg-black/40 border-white/10' : 'bg-black/40 border-white/10'}`}>
      ...
      <div className="hidden lg:flex pointer-events-auto">
        <Magnetic>
          <button
            onClick={() => scrollTo('contact-section')}
            className={`group relative overflow-hidden flex items-center gap-3 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] hover:shadow-[0_0_30px_rgba(37,99,235,0.4)] transition-all duration-500 ${isOnDarkSection ? 'bg-white text-blue-600' : 'bg-white text-blue-600'}`}
          >
            {/* Core Label */}
            <span className="relative z-10 pl-2">Deploy</span>

            {/* Glowing Icon Container */}
            <div className="relative z-10 w-7 h-7 bg-blue-400 rounded-full flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-300">
